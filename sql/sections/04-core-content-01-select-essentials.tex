...
\Pitfall Assuming results are naturally ordered (they aren't). Using \texttt{HAVING} for row filters 
that should be in \texttt{WHERE}. Confusing \texttt{WHERE} vs \texttt{ON} in joins (changes results 
for outer joins).
...
\Pitfall Missing parentheses causing incorrect results. Filtering in the wrong place with outer 
joins (\texttt{WHERE} can turn a \texttt{LEFT JOIN} into an \texttt{INNER JOIN}). Using 
\texttt{NOT IN} when NULLs can appear (often wrong results).
...
\Pitfall Using \texttt{= NULL} instead of \texttt{IS NULL}. \texttt{NOT IN} with NULLs producing 
surprising empty results. Assuming \texttt{COUNT(column)} counts NULLs (it doesn't); use 
\texttt{COUNT(*)} to count rows.
...
\Pitfall Paginating without a stable \texttt{ORDER BY} (duplicate/missing rows across pages). 
Using OFFSET for very deep pages and expecting good performance. Sorting on non-indexed columns 
for large datasets without understanding cost.