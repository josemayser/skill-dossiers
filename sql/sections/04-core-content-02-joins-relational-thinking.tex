...
\Pitfall
\begin{itemize}
  \item Joining tables without understanding cardinality can duplicate rows.
  \item Joining without filtering early can create huge intermediate sets.
\end{itemize}
...
\Pitfall
\begin{itemize}
  \item Using LEFT JOIN but filtering the right table in \texttt{WHERE} drops null-extended rows.
\end{itemize}
...
\Pitfall
\begin{itemize}
  \item Placing conditions in \texttt{WHERE} can turn a LEFT JOIN into an INNER JOIN unintentionally.
\end{itemize}
...
\Pitfall
\begin{itemize}
  \item Using \texttt{DISTINCT} to ''fix'' duplicates without understanding why they happen, hides 
...
\Pitfall
\begin{itemize}
  \item Using \texttt{NOT IN (subquery)} can return empty results if the subquery contains NULL.
\end{itemize}